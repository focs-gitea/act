...
func CompilePattern(pattern string) (*WorkflowPattern, error) {
	if strings.HasPrefix(pattern, "!") {
		rpattern, err := PatternToRegex(pattern[1:])
		if err != nil {
			return nil, err
		}
...
func PatternToRegex(pattern string) (string, error) {
	...
	for pos < len(pattern) {
		...
		case '[':
			...
			validChar := func(a byte, b byte, test byte) bool {
			...
			for pos < len(pattern) && pattern[pos] != ']' {
				...
				case '-':
					...
					validRange := func(a byte, b byte) bool {
...
// return true if the workflow should be skipped paths-ignore/branches-ignore
